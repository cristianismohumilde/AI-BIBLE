...
print("Baixando base para verificar caminhos em outros rolos bíblicos...")
...
try:
    with urllib.request.urlopen(url) as response:
        ...
        for row in reader:
            if not row:
                continue
            if row[bib_idx] == "bib" and row[comp_idx] in ["Deut", "Gen", "Ex", "Ps"]:
                print(f"Book: {row[book_idx]} | Comp: {row[comp_idx]} | Path: {row[path_idx]} | Text: {row[text_idx][:100]}...")
                count += 1
                if count >= 20:
                    break
