...
print("Baixando base para verificar registros com composition vazia...")
...
try:
    with urllib.request.urlopen(url) as response:
        ...
        for row in reader:
            if not row:
                continue
            if row[bib_idx] == "bib" and row[composition_idx] == "":
                print(f"Book: {row[book_idx]} | Composition: {row[composition_idx]} | Path: {row[path_idx]} | Text: {row[text_idx][:100]}...")
                count += 1
                if count >= 15:
                    break
