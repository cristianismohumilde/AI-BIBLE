...
6. **Peshitta Siríaca NT** (Siríaco) — Apenas Novo Testamento (Verificação: confirmada).
   - Fonte Scrollmapper (`Peshitta.json`) baixada para `data/ancient_versions/peshitta_syriac.json`.
   - Estatísticas (checagem automática): **260** capítulos com texto preenchido, **7.956** versículos não vazios (arquivo contém placeholders para outras coleções totalizando 1.189 capítulos na estrutura).
   - Observação: o Antigo Testamento Siríaco (Peshitta OT) não está incluído nesta fonte e requer fonte alternativa caso seja necessário.
